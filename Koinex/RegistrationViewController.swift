    ...
    func registerUser() {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { [self] authResult, error in
            ...
            if error != nil {
                ...
                self.db.collection("users").document(authResult!.user.uid).setData(["firstname": firstName, "lastname":lastName, "mobileno":mobileNo, "uid": authResult!.user.uid, "Wallet": ["totalfunds": 0]]) { (err) in
